...
from .models import DetallesCompras, Compras
...
class DetallesComprasSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['productos', 'cantidad_productos']
...
class ComprasSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'monto_total': {'read_only': True},
            'monto_pagado': {'read_only': True}
        }
    ...
    def create(self, validated_data):
        ...
        product_ids = [detalle['productos'].id for detalle in detalles_data]
        productos = {producto.id: producto for producto in Productos.objects.filter(
        ...
        for detalle in detalles_data:
            producto = productos.get(detalle['productos'].id)

            if not producto:
                errors.append(
                    {'error': f'El producto con ID {detalle["productos"].id} no existe.'})
        ...
        for detalle in detalles_data:
            producto = productos[detalle['productos'].id]
            producto.stock -= detalle['cantidad_productos']

        # Actualizar el stock de todos los productos
        Productos.objects.bulk_update(productos.values(), ['stock'])
